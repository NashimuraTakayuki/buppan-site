...
function addToCart() {
  ...
  if (existingItem) {
    if (existingItem.quantity + qty > maxStock) {
      alert(`すでにカートに ${existingItem.quantity} 個入っています。在庫(${maxStock}個)を超える追加はできません。`);
      return;
    }
    ...
    cart.push({
      ...
      // 会員価格表示用（カート画面での打ち消し線に使う）
      normalPrice: parsePrice(currentSelectedProduct["通常価格"]),
      memberRate:  parseFloat(currentSelectedProduct["会員価格(割合%)"]) || 0,
      quantity:    qty,
      maxStock:    maxStock,
    });
  }
...
function renderEditableCart() {
  ...
  cart.forEach((item, index) => {
    ...
    // LINE連携済み かつ 通常価格あり かつ 会員割引設定あり の場合のみ打ち消し線表示
